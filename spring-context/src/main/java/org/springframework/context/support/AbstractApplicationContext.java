...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	protected void prepareRefresh() {
		 ...
		 * 如下示例，在Web的容器加载中，会初始化ConfigurableEnvironment的配置
		...
		//证是否系统环境中有RequiredProperties参数值,验证是否存在需要的属性(标注为必须的属性值，获取为空，则抛出异常)
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		/**
			销毁原工厂，并重新创建工厂。主要工作是创建DefaultListableBeanFactory实例
		 	创建的后调用loadBeanDefinitions ，解析配置文件，注册Bean的定义信息，如下所示
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 下面注册的ResourceEditorRegistrar，会注册资源相关的一批属性解析器
		 ...
		 * 因为{@link ApplicationContextAwareProcessor#invokeAwareInterfaces}把这5个接口的实现工作做了
		 */
		...
		/**
		  ...
		  其它类似的Bean，还有WebApplicationContextUtils中会设置的request、response、session等相关对象)等，
		 	见：{@link org.springframework.web.context.support.WebApplicationContextUtils#registerWebApplicationScopes}
		*/
